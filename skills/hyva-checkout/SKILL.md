---
name: hyva-checkout
description: Build and debug Hyvä Checkout on Magento 2 — Magewire components, hyva_checkout.xml steps, the Form API, window.hyvaCheckout, the Evaluation API and Place Order Services, and mandatory strict CSP. Use when adding or reordering checkout steps, building or debugging a Magewire component, changing shipping/billing address fields or adding a custom address EAV attribute, writing an EntityFormModifierInterface, registering a payment or shipping method renderer, porting a Luma/Knockout payment module, writing a Place Order Service or a 3DS/redirect flow, hooking checkout JS events, or troubleshooting Magewire 404s, wrong totals, "Main wire element could not be found", or inline scripts blocked by CSP. Applies to any work on the checkout steps, cart-to-order flow, or payment/shipping method UI of a Hyvä store even when Hyvä is not named — keywords hyva_checkout.xml, wire:model, wire:auto-save, EvaluationResultFactory, PlaceOrderServiceProvider, registerMethod, hyvaCheckout.api.
---
