---
name: hyva-admin
description: Builds and customizes Magento 2 adminhtml grids declaratively with the Hyva_Admin module (hyva-themes/module-magento2-admin) instead of Magento UI Components — a single grid XML file in view/adminhtml/hyva-grid/ plus a Hyva\Admin\Block\Adminhtml\HyvaGrid block in layout XML gives columns, paging, sorting, filters, row actions, mass actions, buttons and CSV/XML/XLSX exports, with data coming from a repository getList method, an ORM collection, a plain array provider or a SQL select declared in XML. Use this skill when asked to add or change an admin grid, list records in the Magento backend, replace or avoid a ui_component listing XML, choose a grid data source, add a column renderer/template/type, enable a column filter or write a custom filter type, wire row or mass actions to a controller, add a grid export format, join extra fields into grid data, hook grid loading with a grid source processor or one of the hyva_grid_* events, or when the code mentions hyva-grid.xsd, HyvaGridArrayProviderInterface, HyvaGridSourceProcessorInterface, arrayProvider, repositoryListMethod, defaultSearchCriteriaBindings, massActions or Hyva_Admin.
---
